...
struct ILStatisticsScreen: View {
    ...
    private func statCell(title: String, value: String, symbol: String, tint: Bool, interactive: Bool, action: @escaping () -> Void) -> some View {
        let cellTint = tint ? ILTheme.amber : rewardAccent.light
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(
                    LinearGradient(colors: [cellTint.opacity(0.20), ILTheme.divider], startPoint: .topLeading, endPoint: .bottomTrailing),
                    lineWidth: 1
                )
        )
