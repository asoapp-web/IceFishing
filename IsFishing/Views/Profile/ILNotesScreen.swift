...
struct ILNotesScreen: View {
    ...
    var body: some View {
        ZStack(alignment: .bottomTrailing) {
            ILGradient.vertical(ILTheme.iceLight, ILTheme.frostWhite)
                .ignoresSafeArea()
            if sorted.isEmpty {
                ILEmptyState(icon: "note.text.badge.plus", message: "No notes yet. Tap + to jot down your first thought.", onDark: false)
            } else {
                ScrollView {
                    LazyVStack(spacing: 12) {
                        ForEach(sorted) { n in
                            Button {
                                ...
                                .padding(14)
                                .background(RoundedRectangle(cornerRadius: 16).fill(ILTheme.frostWhite))
            ...
            Button {
                ...
                Image(systemName: "plus")
                    ...
                    .background(Circle().fill(accent.mid))
            }
            .padding()
        }
